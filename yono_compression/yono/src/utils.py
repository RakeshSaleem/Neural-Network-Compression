...
def use_mixup(bool):
    return 'usemixup' if bool else 'nomixup'

def use_pyquantize(bool):
    return 'pyquant' if bool else ''
...
def mixup_loss(args, model, pred, y_a, y_b, lam):
    return lam * regularized_nll_loss(args, model, pred, y_a) + (1 - lam) * regularized_nll_loss(args, model, pred, y_b)


def regularized_nll_loss_mtl(args, model, output, target, t, num_batches):
    # hack
    # w1 = 1.e-3
    # w2 = 1.e-3
    w3 = 5.e-6
    ...
    index = 0
    loss = w3 * F.nll_loss(output, target)
    if args.l2:
